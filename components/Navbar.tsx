              ...
              return (
                <div key={item.label} className="relative group">
                  <Link
                    ...
                    className={`font-body text-sm font-semibold tracking-wider uppercase transition-all duration-300 flex items-center gap-1 py-2 relative ${
                      isTransparent
                        ? active ? 'text-white' : 'text-white/90 hover:text-white'
                        : active ? 'text-teal' : 'text-slate-800 hover:text-teal'
                    }`}
                  >
                    {item.label}
                    {item.children && (
                      ...
                      <div className="bg-white rounded-xl shadow-premium border border-cream overflow-hidden">
                        {item.children.map((child) => (
                          <Link
                            key={child.label}
                            to={child.url}
                            className={`block px-6 py-4 text-sm font-semibold transition-colors border-b border-cream last:border-0 ${
                              isActivePath(child.url)
                                ? 'bg-cream-light text-teal'
                                : 'text-slate-800 hover:bg-cream-light hover:text-teal'
                            }`}
                          >
                            {child.label}
                          </Link>
                ...
                {item.children ? (
                  <div className="flex flex-col items-center">
                    <button
                      ...
                      className={`font-heading text-3xl transition-all duration-300 flex items-center gap-2 ${
                    ...
                    <div className={`overflow-hidden transition-all duration-500 w-full ${mobileExpandedItem === item.label ? 'max-h-64 mt-4' : 'max-h-0'}`}>
                      {item.children.map((child) => (
                        <Link
                          key={child.label}
                          to={child.url}
                          className={`block py-3 font-body text-xl transition-colors ${
                            isActivePath(child.url) ? 'text-gold' : 'text-white/60 hover:text-gold'
                          }`}
                        >
                          {child.label}
                        </Link>
                  ...
                  <Link
                    to={item.url}
                    aria-current={active ? 'page' : undefined}
                    className={`font-heading text-4xl transition-all duration-300 block transform ${
